...
def allocate_groups_random(members_list):
    ...
    groups = [members_list[i:i+4] for i in range(0, len(members_list), 4)]
...
tab1, tab2 = st.tabs(["Group Allocation", "Score Collection"])
...
with tab1:
    ...
    if st.session_state.groups:
        ...
        tabs = st.tabs([f"Group {i+1}" for i in range(len(st.session_state.groups))])
        
        for i, (tab, group) in enumerate(zip(tabs, st.session_state.groups)):
            with tab:
                    ...
                    if player in st.session_state.scores:
                        ...
                        for hole in range(1, 10):
                            ...
                            with cols[hole]:
                                ...
                                display_value = 1 if current_value is None else int(current_value)
